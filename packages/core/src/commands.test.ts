...
describe('CommandRegistry', () => {
    ...
    const keyEvent = (key: string, mods: { meta?: boolean; ctrl?: boolean; shift?: boolean; alt?: boolean } = {}): KeyboardEvent => ({
        key,
    ...
    describe('execute', () => {
        it('executes a registered handler', async () => {
            ...
            registry.register(id, () => { called = true; });
    ...
    describe('executeFromShortcut', () => {
        ...
        it('requires exact modifier match', async () => {
            ...
            registry.register(id, () => { callCount++; });

            expect(await registry.executeFromShortcut(keyEvent('a', { meta: true }))).toBe(false);
            expect(await registry.executeFromShortcut(keyEvent('a', { meta: true, shift: true }))).toBe(true);
